...
function GameController() {
    ...
    const gameOver = () => {
        ...
        for (i = 1; i < row; i++) {
            if ((gameBoard.getCellValue(0, 0) !== gameBoard.getCellValue(i, i)) || gameBoard.getCellValue(i, i) === null)
                break;
        }
        if (i === row)
        ...
        for (i = 1; i < row; i++) {
            if ((gameBoard.getCellValue(0, row - 1) !== gameBoard.getCellValue(i, row - i - 1)) || gameBoard.getCellValue(i, row - i - 1) === null)
                break;
        }
        if (i === row)
            gameDetails.gameOver = true;
        ...
        for (i = 0; i < row; i++) {
            for (j = 1; j < column; j++) {
                if ((gameBoard.getCellValue(i, 0) !== gameBoard.getCellValue(i, j)) || gameBoard.getCellValue(i, j) === null)
                    break;
            }
            if (j === column)
                gameDetails.gameOver = true;
        ...
        for (j = 0; j < column; j++) {
            for (i = 1; i < row; i++) {
                if ((gameBoard.getCellValue(0, j) !== gameBoard.getCellValue(i, j)) || gameBoard.getCellValue(i, j) === null)
                    break;
            }
            if (i === row)
                gameDetails.gameOver = true;
...
function UIController() {
    ...
    const updateScreen = (roundDetails) => {
        ...
        if (roundDetails.validMove) {
            updateGameBoardDiv(roundDetails.r, roundDetails.c, roundDetails.playedBy.getToken())
        }
